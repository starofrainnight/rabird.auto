...
class Keyboard(common.Keyboard):
    ...
    ##
    # Dict to indicate if a virtual key code is extended code.
    #
    # If return value less than 256 that mean it's a extended prefix for
    # real scancode, you must contact it with that extended prefix.
    #
    # If return value large than or equal 256, that means it should
    # replace the original scancode.
    #
    # For example :
    #
    # @code
    # scancode = win32api.MapVirtualKey(vkcode, 0)
    # extended_prefix = __vk_to_sc_ex_dict[win32con.VK_RCONTROL]
    # if extended_prefix <= 255:
    #     scancode = (extended_prefix << 8) | scancode
    # @endcode
    #
    ...
    # keyboards
    __vk_to_sc_ex_dict = {
        win32con.VK_RCONTROL: 0xE0,
        win32con.VK_RMENU: 0xE0,  # ALT
        win32con.VK_DELETE: 0xE0,
        win32con.VK_UP: 0xE0,
        win32con.VK_DOWN: 0xE0,
        win32con.VK_LEFT: 0xE0,
        win32con.VK_RIGHT: 0xE0,
        win32con.VK_HOME: 0xE0,
        win32con.VK_END: 0xE0,
        win32con.VK_INSERT: 0xE0,
        win32con.VK_CLEAR: 0xE0,
        win32con.VK_OEM_CLEAR: 0xE0,
        win32con.VK_PRIOR: 0xE0,  # Page Up
        win32con.VK_NEXT: 0xE0,
        win32con.VK_SELECT: 0xE0,
        0x5F: 0xE0,  # 'SLEEP'
        win32con.VK_EXECUTE: 0xE0,
        win32con.VK_HELP: 0xE0,
        win32con.VK_APPS: 0xE0,
        win32con.VK_PRINT: 0xE02AE037,
        win32con.VK_LWIN: 0xE0,  # WIN
        win32con.VK_RWIN: 0xE0,
        win32con.VK_CANCEL: 0xE0,  # BREAK
        win32con.VK_PAUSE: 0xE11D45E19DC5,
        win32con.VK_DIVIDE: 0xE0,
        win32con.VK_RETURN: 0xE0,

        win32con.VK_PLAY: 0xE0,
        win32con.VK_ZOOM: 0xE0,
        win32con.VK_PA1: 0xE0,

        win32con.VK_BROWSER_BACK: 0xE0,  # BROWSER_BACK
        win32con.VK_BROWSER_FORWARD: 0xE0,
        0xA8: 0xE0,  # BROWSER_REFRESH
        0xA9: 0xE0,  # BROWSER_STOP
        0xAA: 0xE0,  # BROWSER_SEARCH
        0xAB: 0xE0,  # BROWSER_FAVORITES
        0xAC: 0xE0,  # BROWSER_HOME

        win32con.VK_VOLUME_MUTE: 0xE0,
        win32con.VK_VOLUME_DOWN: 0xE0,
        win32con.VK_VOLUME_UP: 0xE0,

        win32con.VK_MEDIA_NEXT_TRACK: 0xE0,
        win32con.VK_MEDIA_PREV_TRACK: 0xE0,
        0xB2: 0xE0,  # MEDIA_STOP
        0xB3: 0xE0,  # MEDIA_PLAY_PAUSE

        0xB4: 0xE0,  # LAUNCH_MAIL
        0xB5: 0xE0,  # LAUNCH_MEDIA_SELECT
        #         'LAUNCH_APP1':[0xB6],
        #         'LAUNCH_APP2':[0xB7],
    ...
    def _vk_to_sc_ex(self, vkcode):
        if vkcode in self.__vk_to_sc_ex_dict:
            return self.__vk_to_sc_ex_dict[vkcode]
        else:
            return 0

    def map_vk_to_sc(self, vkcode):
        scancode = win32api.MapVirtualKey(vkcode, 0)

        if int(platform.version().split(".")[0]) > 5:
            # MAPVK_VK_TO_VSC_EX  = 4, returned high byte will be 0xE0 or 0xE1
            # Only work with os after vista
            extended_scancode = win32api.MapVirtualKey(
                vkcode, 4, win32api.GetKeyboardLayout())
            if (scancode != extended_scancode) and (extended_scancode != 0):
                scancode = extended_scancode
        else:
            # MapVirtualKeyEx will crash on vista below, so we needs another
            # way to check the extend code.
            extended_scancode = self._vk_to_sc_ex(vkcode)
            if extended_scancode != 0:
                if extended_scancode <= 255:
                    scancode = ((extended_scancode << 8) | scancode)
                else:
                    scancode = extended_scancode
    ...
    def _send_method(self, action, context):
        ...
        if int == type(vkcode):
            scancode = self.map_vk_to_sc(vkcode)
            flags = 0
            if scancode > 255:
